...
import { runOptimalAlgorithm, OptimalResult } from "@/utils/optimalAlgo";
import { styles } from "@/styles/fifo.styles";
import { useMutation } from "convex/react";
import { useUser } from "@clerk/clerk-expo";
import { api } from "@/convex/_generated/api";
import { incrementOptimal } from "@/convex/increment";

const OptimalScreen = () => {
  const [inputString, setInputString] = useState("");
  const [frameSize, setFrameSize] = useState("");
  const [result, setResult] = useState<OptimalResult | null>(null);
  const incrementOptimal = useMutation(api.increment.incrementOptimal); // Update path if needed
const { user } = useUser();
  ...
  const handleAnalyze = async () => {
    ...
    if (pages.length && frame > 0) {
      const output = runOptimalAlgorithm(pages, frame);
      setResult(output);
           // Increment LRU count in database
    if (user?.id) {
      try {
        await incrementOptimal({ clerkId: user.id });
      } catch (error) {
  ...
  const handlePrint = async () => {
    ...
    const html = `
      <h1>Optimal Page Replacement Result</h1>
      ...
      <table border="1" style="border-collapse: collapse; width: 100%; text-align: center;">
        <thead>
          <tr>
            <th>Step</th>
            <th>Page</th>
            ${result.steps[0]?.frame
              .map((_, i) => `<th>Frame ${i + 1}</th>`)
              .join("")}
            <th>Status</th>
          </tr>
        </thead>
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.heading}>Optimal Page Replacement</Text>

        {/* Input Section */}
        <View style={styles.inputContainer}>
          <TextInput
            style={styles.input}
            placeholder="Enter pages (e.g. 7, 0, 1, 2, 0, 3)"
        ...
        {result && (
          <>
            ...
            <View style={styles.statsContainer}>
              ...
              <Text style={styles.statText}>
                Page Misses: <Text style={styles.missText}>{result.pageMisses}</Text>
              </Text>
              <Text style={styles.statText}>
                Hit Ratio:{" "}
                <Text style={styles.ratioText}>{result.hitRatio.toFixed(2)}%</Text>
              </Text>
            </View>

            {/* Explanation */}
            <View style={styles.infoCard}>
              <Text style={styles.infoTitle}>Optimal Algorithm</Text>
              <Text style={styles.infoText}>
                Optimal page replacement replaces the page that will not be used
                for the longest time in the future. It gives the best possible
                performance but requires future knowledge of requests.
              </Text>
