...
import { runLruAlgorithm, LruResult } from "@/utils/lruAlgo";
...
const Lru = () => {
  ...
  const incrementLru = useMutation(api.increment.incrementLru); // Update path if needed
const { user } = useUser();
  ...
  const handleAnalyze = async () => {
    ...
    if (pages.length && frame > 0) {
      const output = runLruAlgorithm(pages, frame);
      setResult(output);
       // Increment LRU count in database
    if (user?.id) {
      try {
        await incrementLru({ clerkId: user.id });
      } catch (error) {
  ...
  const handlePrint = async () => {
    ...
    const html = `
      <h1>LRU Page Replacement Result</h1>
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.heading}>LRU Page Replacement</Text>
          ...
          <>
            ...
            <View style={styles.statsContainer}>
              <Text style={styles.statText}>
                Page Hits:{" "}
                <Text style={styles.hitText}>{result.pageHits}</Text>
              </Text>
              <Text style={styles.statText}>
                Page Misses:{" "}
            ...
            <View style={styles.infoCard}>
              <Text style={styles.infoTitle}>LRU Algorithm</Text>
              <Text style={styles.infoText}>
                LRU (Least Recently Used) replaces the page that hasn’t been
                used for the longest time. It tracks page usage with timestamps.
              </Text>
